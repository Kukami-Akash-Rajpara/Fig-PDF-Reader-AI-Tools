...
import com.app.figpdfconvertor.figpdf.databinding.ActivityLoadingHiringBinding;
...
public class LoadingHiringActivity extends AppCompatActivity {

    private ActivityLoadingHiringBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityLoadingHiringBinding.inflate(getLayoutInflater());
        ...
        if (pdfUriString != null) {
            uploadPdf(Uri.parse(pdfUriString), jobDescription);
        } else {
    ...
    private void uploadPdf(Uri pdfUri, String jobDescription) {
        try {
            ...
            RequestBody jdTextBody = RequestBody.create(jobDescription, MultipartBody.FORM);

            Log.d("Akash", "JD Text -> " + jobDescription);
            Log.d("Akash", "File -> " + pdfFile.getName());

            analyzerHiringCall = ApiClient.INSTANCE.getApiService().getHiring(BuildConfig.VERSION_CODE, body, jdTextBody);
            analyzerHiringCall.enqueue(new Callback<>() {
                ...
                public void onResponse(@NonNull Call<ResumeAnalyzerHiring> call, @NonNull Response<ResumeAnalyzerHiring> response) {
                    ...
                    if (response.isSuccessful() && response.body() != null) {
                        ...
                        Intent intent = new Intent(LoadingHiringActivity.this, ResumeDetailsActivity.class);
