...
class CurvedBottomNavigationView @JvmOverloads constructor(
    ...
    var fabBackgroundDrawable: Drawable? = null
        set(value) {
            field = value
            if (isMenuInitialized) {
                invalidate()
        ...
        context.theme.obtainStyledAttributes(attrs, R.styleable.CurvedBottomNavigationView, 0, 0)
            .apply {
                try {
                    ...
                    fabBackgroundDrawable = getDrawable(R.styleable.CurvedBottomNavigationView_cbn_fabBg)
...
//         pop to the navigation graph's start  destination
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (fabIconIndex == 0) {
            val pngDrawable = ResourcesCompat.getDrawable(resources, R.drawable.active_home, context.theme)
            pngDrawable?.let {
                // Scale factor for slightly bigger image
                val scaleFactor = 1.3f
                val radius = fabSize / 2f * scaleFactor

                val left = (centerX - radius).toInt()
                val top = (curCenterY - radius).toInt()
                val right = (centerX + radius).toInt()
                val bottom = (curCenterY + radius).toInt()

                it.setBounds(left, top, right, bottom)
                it.draw(canvas)
            }
        } else {
            if (fabBackgroundDrawable != null) {
                val left = (centerX - fabSize / 2).toInt()
                val top = (curCenterY - fabSize / 2).toInt()
                val right = (centerX + fabSize / 2).toInt()
                val bottom = (curCenterY + fabSize / 2).toInt()
                fabBackgroundDrawable?.setBounds(left, top, right, bottom)
                fabBackgroundDrawable?.draw(canvas)
            } else {
                canvas.drawCircle(centerX, curCenterY, fabSize / 2f, fabPaint)
            }
//            canvas.drawCircle(centerX, curCenterY, fabSize / 2f, fabPaint)
            // Draw normal icon (previous behavior)
        ...
        // Draw the bottom navigation path
        canvas.drawPath(path, navPaint)

        // Draw dots/text for other bottom nav items
